...
11. A comparable profile depends on the same runner image reference and ID, architecture, Node version, npm version, `strace` version, sandbox, coverage scope, and observation policy. The generated dependency lock digest records, but does not eliminate, dependency graph variation.
