...
func TestTraceArgumentsKeepPackageSpecAfterImage(t *testing.T) {
	...
	arguments := buildTraceArgs("behaviorlock-trace-abc", "behaviorlock-analysis:abc", packageSpec)
...
func TestPrepareArgumentsNeverMountHostPaths(t *testing.T) {
	t.Parallel()
	arguments := buildPrepareArgs("behaviorlock-prep-abc", "safe-package@1.2.3")
...
func TestParsePrepareMetadata(t *testing.T) {
	...
	if metadata.Integrity != "sha512-abc" {
...
func TestCaptureCompletesOnlyWithValidEnvelope(t *testing.T) {
	...
	runner.run = func(_ context.Context, arguments []string, _, _ int64) (commandResult, error) {
		...
		case "image":
			if arguments[1] == "inspect" {
				if arguments[len(arguments)-1] == "{{.Architecture}}" {
					return commandResult{Stdout: []byte("amd64\n")}, nil
				}
				return commandResult{Stdout: []byte("sha256:runner-image\n")}, nil
			}
			return commandResult{}, nil
		case "create", "commit", "rm":
			return commandResult{}, nil
		case "start":
			return commandResult{Stdout: []byte("BEHAVIORLOCK_PREP_V1 {\"integrity\":\"sha512-test\",\"dependencyLockSha256\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}\n")}, nil
		case "run":
	...
	if profile.Subject.RegistryIntegrity != "sha512-test" || profile.Capture.RunnerImageID != "sha256:runner-image" {
